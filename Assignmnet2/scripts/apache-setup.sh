...
# ---------------------------
# Open firewall port 80 if firewalld exists
# ---------------------------
...
# ---------------------------
if [ -n "$PUBLIC_IP" ]; then
    echo "🌐 Open your backend page in a browser: http://$PUBLIC_IP"
else
    echo "⚠️ Public IP not available. Use private IP or check EC2 network settings."
fi
